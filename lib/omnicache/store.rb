...
module OmniCache
  class Store # :nodoc:
    ...
    def read_multi(*keys)
      with_tracing("read_multi", key: keys.join(", ")) do
    ...
    def write_multi(entries, ttl_seconds: nil)
      with_tracing("write_multi", key: entries.keys.join(", ")) do
        results = maybe_threadsafe do
    ...
    def delete(key)
      with_tracing("delete", key: key) do
        maybe_threadsafe do
